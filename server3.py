...
BACKLOG = 50
MAX_THREADS = 200
#BLACKLISTED = [www.netflix.com:443]
BLACKLISTED = ['www.netflix.com']
...
class ConnectionHandle(threading.Thread):
	...
	def run(self):
		...
		print(type(req.host))
		print(BLACKLISTED)
		print(len(BLACKLISTED[0]))
		print(len(req.host))
		if str(req.host) in BLACKLISTED:
			print("Hello")
		...
		try:
			if req.path in cache:
				...
				cached_response = cache[req.path]
				self.client_conn.send(cached_response)
				# for i in cached_response:
				# 	print(len(cached_response))
				# 	self.client_conn.send(i)
				#print(cached_response[4])
				self.client_conn.close()
				#print("----------bhoot----------")
				logg.info(f"{req.method:<8} {req.path} {req.protocol} SERVED FROM CACHE")
		...
		while True:
			...
			try:
				...
				if self.server_conn in triple:
					...
					# if(req.path in cache):
					# 	cache[req.path].append(data)
					# else:
					# 	cache[req.path]=[]
					cache[req.path]=data
					# print(req.path)
					# print(data)
					# print(cache[req.path], "---anish---")
...
if __name__ == '__main__':
	ser = Server(host="localhost", port=8000)
	ser.start()
