	...
	def run(self):
		...
		if ('accept-language' not in headers) or (headers['accept-language']=='en'):
			...
			if request.path == "/":
				...
				self.client_conn.close()
				logg.info(f"{request.method:<8} {request.path} {request.protocol} SERVED FROM SERVER x")
				return
				...
				self.client_conn.send(data)
				self.client_conn.close()
				logg.info(f"{request.method:<8} {request.path} {request.protocol} SERVED FROM SERVER 1")
				return
				...
				self.client_conn.send(data)
				self.client_conn.close()
				logg.info(f"{request.method:<8} {request.path} {request.protocol} SERVED FROM SERVER 2")
				return
			...
			self.client_conn.send(data)
			self.client_conn.close()
			logg.info(f"{request.method:<8} {request.path} {request.protocol} SERVED FROM SERVER 3")
			return
